...
func rewriteValueARM64_OpZero(v *Value) bool {
	...
	// match: (Zero [s] ptr mem)
	// cond: s%32 != 0 && s%32 > 8 && s%32 <= 16 && s > 16
	// result: (Zero [16] (OffPtr <ptr.Type> ptr [s-16]) (Zero [s-s%32] ptr mem))
	for {
		s := auxIntToInt64(v.AuxInt)
		ptr := v_0
		mem := v_1
		if !(s%32 != 0 && s%32 > 8 && s%32 <= 16 && s > 16) {
			break
		}
		v.reset(OpZero)
		v.AuxInt = int64ToAuxInt(16)
	...
	// match: (Zero [s] ptr mem)
	// cond: s%32 != 0 && s%32 > 16 && s%32 <= 24 && s > 16
	// result: (Zero [24] (OffPtr <ptr.Type> ptr [s-24]) (Zero [s-s%32] ptr mem))
	for {
		s := auxIntToInt64(v.AuxInt)
		ptr := v_0
		mem := v_1
		if !(s%32 != 0 && s%32 > 16 && s%32 <= 24 && s > 16) {
			break
		}
		v.reset(OpZero)
		v.AuxInt = int64ToAuxInt(24)
