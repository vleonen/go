...
func TestEmitRelocsCompressedDWARF(t *testing.T) {
	testenv.MustHaveGoBuild(t)
	testenv.MustHaveCGO(t)
	t.Parallel()
...
`)
	...
	compressedDebug := make(map[string]*elf.Section)
	for _, sec := range f.Sections {
		if sec.Flags&elf.SHF_COMPRESSED != 0 && strings.HasPrefix(sec.Name, ".debug_") {
			compressedDebug[sec.Name] = sec
		}
	}
	if len(compressedDebug) == 0 {
		t.Fatal("expected at least one compressed .debug_* section, got none")
	}

	for name, sec := range compressedDebug {
		if sec.Size == 0 {
			t.Errorf("compressed section %s has size 0, expected non-zero", name)
		}
	}

	relaSections := make(map[string]*elf.Section)
	for _, sec := range f.Sections {
		if sec.Type == elf.SHT_RELA && strings.HasPrefix(sec.Name, ".rela.debug_") {
			relaSections[sec.Name] = sec
		}
	}

	for relaName, relaSec := range relaSections {
		targetName := relaName[len(".rela"):]
		targetSec, ok := compressedDebug[targetName]
		if !ok {
			continue
		}
		if targetSec.Flags&elf.SHF_COMPRESSED != 0 && relaSec.Size != 0 {
			t.Errorf("%s has size %d targeting compressed section %s, want size 0",
				relaName, relaSec.Size, targetName)
		}
	}
}
