...
func ssaGenValue(s *ssagen.State, v *ssa.Value) {
	...
	case ssa.OpARM64LoweredMove:
		...
		var off int64
		// TODO: >=32 case uses only F registers, while <32 case uses only R registers
		// We can split LoweredMove into two (>=32 and <32) to reduce registers spilling
		if n >= 32 {
			for n >= 32 {
				//  FLDPQ   off(srcReg), (tmpFReg1, tmpFReg2)
				//  FSTPQ   (tmpFReg1, tmpFReg2), off(dstReg)
				move32f(s, srcReg, dstReg, tmpFReg1, tmpFReg2, off, false)
				off += 32
				n -= 32
			}
			if n != 0 {
				// Copy reminder with FLDPQ/FSTPQ/FMOVQ with adjusting
				// offset to write overlapped reminder
				if n > 16 {
					off -= 32 - n
					//  FLDPQ   off(srcReg), (tmpFReg1, tmpFReg2)
					//  FSTPQ   (tmpFReg1, tmpFReg2), off(dstReg)
					...
					//  FMOVQ     off(src), tmp1
					//  FMOVQ     tmp1, off(dst)
					move16f(s, srcReg, dstReg, tmpFReg1, off, false)
				}
			}
		} else {
			for n >= 16 {
	...
	case ssa.OpARM64LoweredMoveLoop:
		...
		for range loopSize / 32 {
			...
			move32f(s, srcReg, dstReg, tmpFReg1, tmpFReg2, 0, true)
		}
		// Decrement loop count.
		//   SUB     $1, countReg
		p = s.Prog(arm64.ASUB)
...
// move32f copies 32 bytes at src+off to dst+off.
// Uses fp registers tmp1 and tmp2.
// If postInc is true, increment src and dst by 32.
func move32f(s *ssagen.State, src, dst, tmp1, tmp2 int16, off int64, postInc bool) {
...
// move16f copies 16 bytes at src+off to dst+off.
// Uses fp register tmp1
// If postInc is true, increment src and dst by 16.
func move16f(s *ssagen.State, src, dst, tmp1 int16, off int64, postInc bool) {
	// FMOVQ     off(src), tmp1
